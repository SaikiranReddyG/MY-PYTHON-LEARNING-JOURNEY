...
def valid_space(shape, grid):
    accepted_positons = [[(j, i) for j in range(10) if grid[i][j] == (0, 0, 0)] for i in range(20)]
    accepted_positons = [j for sub in accepted_positons for j in sub]

    formatted = convert_shape_format(shape)

    for pos in formatted:
        if pos not in accepted_positons:
            if pos[1] > -1:
                return False
...
def draw_window(surface):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            pygame.draw.rect(surface, grid[i][j], (top_left_x + j * block_size, top_left_y + 1 * block_size, block_size, block_size), 0)
...
def main():
    ...
    while run:
        ...
        if check_lost(locked_position):
            run = False
    # draw_text_middle(win, "YOU LOST", 80, (255,255,255),0)
    pygame.display.update()
